...
- `notes/parallelism.md` - Design rationale for the parallelism approach
- `notes/pumps-and-passes.md` - The pass/pump distinction
- `notes/no-caching.md` - Why interestingness-test caching is mostly useless here
- `notes/nondeterminism.md` - Handling of nondeterministic interestingness tests (detection, gauntlet, backtracking)
- `notes/performance-programme-2026-09.md` - What the September 2026 performance investigation kept, rejected (an alternative retry-budget search) and left unexplained
- `notes/import-graph.md` - Module import graph (auto-generated by `scripts/update_import_graph.py`)
...
5. **Nondeterministic tests**: `BasicReductionProblem` detects a nondeterministic interestingness test by replaying the initial test case at setup, the current one periodically, and the result at the end; once detected, candidates must clear a statistical gauntlet (`nondeterminism.py`) before adoption and the run backtracks through its history to a reproducing test case. See `notes/nondeterminism.md`.
6. **Adaptive timeouts**: `AdaptiveTimeoutPolicy` (`adaptive_timeout.py`) chooses each test run's timeout from recent measured runtimes (user `--timeout` is the maximum). When reduction stalls with tests timing out it raises the timeout to look for slow reductions; cached timeout-failures carry a validity condition so they are retried after a raise, and reducers call `problem.attempt_unstick()` before terminating so a raised timeout can trigger another round.
...
- **Unit tests** (`tests/test_tui.py`) use `FakeReductionClient` to test TUI logic without spawning subprocesses
