- Shrink Ray now has an LLM mode, enabled by default: alongside the ordinary
  ...
  propose smaller test cases once the other passes stop making progress. Model
  suggestions are candidates like any others — they're only accepted if your
  interestingness test passes — so a bad model costs time but never
  correctness.
