...
test *args: install
    {{ if args == "" { "uv run --extra dev pytest tests -n auto -m 'not slow and not serial' --durations=10 --cov --cov-report= && uv run --extra dev pytest tests -n 0 -m 'serial and not slow' --cov --cov-append --cov-report=term-missing --cov-fail-under=100" } else { "uv run pytest " + args } }}
