...
def test_subprocess_client_drain_stderr_returns_early_when_process_is_none():
    """Test _drain_stderr returns early when process is None."""

    async def run():
        client = SubprocessClient()
        # _drain_stderr should return immediately when process is None
...
def test_subprocess_client_drain_stderr_returns_early_when_stderr_is_none():
    """Test _drain_stderr returns early when stderr is None."""
    from unittest.mock import MagicMock

    async def run():
        client = SubprocessClient()

        # Create a mock process with no stderr
        mock_process = MagicMock()
        mock_process.stderr = None
        client._process = mock_process

        # _drain_stderr should return immediately when stderr is None
        await client._drain_stderr()
        # No exception means success
...
def test_subprocess_client_drain_stderr_handles_exception():
    """Test _drain_stderr handles exceptions during read gracefully."""
    from unittest.mock import MagicMock

    async def run():
        client = SubprocessClient()

        # Create a mock process with stderr that raises an exception
        class FailingStderr:
            async def read(self, n):
                raise ConnectionResetError("Connection lost")

        mock_process = MagicMock()
        mock_process.stderr = FailingStderr()
        client._process = mock_process
...
def test_subprocess_client_does_not_deadlock_on_stderr(tmp_path):
    """Test that SubprocessClient doesn't deadlock when worker writes to stderr.

    This tests a bug where stderr was captured but never read, causing the
    subprocess to block when its stderr buffer filled up (~64KB).
    """
    import sys
    import textwrap
    from unittest.mock import patch
    ...
    async def run():
        # Create a script that writes to stderr in a blocking loop
        # It writes more than the pipe buffer (64KB) then sends response
        blocking_script = tmp_path / "blocking_stderr.py"
        blocking_script.write_text(
            textwrap.dedent("""
            import sys
            import json
            ...
            # Now try to write 1MB to stderr - this will block if stderr
            # pipe fills up and nobody is reading it
            for _ in range(1024):
                sys.stderr.write('x' * 1024)
                sys.stderr.flush()

            # Send response - we should reach here if stderr is being drained
            response = {"id": request["id"], "result": {"status": "ok"}, "error": None}
        ...
        with patch.object(SubprocessClient, "start", patched_start):
            client = SubprocessClient()
            await client.start()

            try:
                # Send a request using send_command which the client reads
